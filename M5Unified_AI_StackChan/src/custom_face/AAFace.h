  ...
  */
  ...
  public:
    ...
    void draw(M5Canvas *spi, BoundingRect rect, DrawContext *ctx)
    {
      ...
      uint32_t offset_y = 25;

      // speech mode
      float openRatio = ctx->getMouthOpenRatio();
      uint32_t h = (minHeight + (maxHeight - minHeight) * openRatio)*pow(cos(openRatio), 3.0)*3.5;
      uint32_t mx = 162;
      uint32_t my = 115 + offset_y;
      uint32_t mrx = 13;
      if (h > minHeight) {
  ...
  public:
    AAFace()
        : Face(new AAMouth(), new BoundingRect(74, 136),
               new AAEye(8, false), new BoundingRect(93 + y_offset, 90),
               new AAEye(8, true), new BoundingRect(96 + y_offset, 230),
               new AAEyeblow(18, 3, false), new BoundingRect(71 + y_offset, 50),
               new AAEyeblow(18, 3, true), new BoundingRect(73 + y_offset, 270)) {}
  };
